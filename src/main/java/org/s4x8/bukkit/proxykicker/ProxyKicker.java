...
public class ProxyKicker extends JavaPlugin {
	...
	private void loadPorts() {
		...
		Iterator<String> it = getConfig().getStringList("proxy-ports").iterator();
		while (it.hasNext()) {
			int port = -1;
			...
			if (port >= 1 && port <= 65535) {
				...
				getLogger().warning(portString + " is not a valid port!");
			};
		};
	};
	...
	private void initializeTor() {
		...
		if (updateInterval <= 0L) {
			...
			if (lastUpdate < 0L) update = true;
			if ((new Date()).getTime() - lastUpdate >= updateInterval) update = true;
		};
	...
	private void downloadTor() {
		...
		while (it.hasNext()) {
			...
			try {
				...
				logger.warning("Invalid Tor list URL: " + stringUrl);
			};
			...
			if (url != null) {
				...
				try {
					...
					try {
						if (loadStream != null) loadStream.close();
					} catch (Exception e) { };
				};
			};
		};
